        ...
        return (
          ...
          >
            ...
            <div className="relative">
              ...
              {/* badge hanya muncul kalau ada unread */}
              {badge && unreadCount > 0 && (
...
export default function DashboardOwnerPage() {
  ...
  // fetch listings (tidak berubah)
  useEffect(() => {
    (async () => {
      try {
        const res  = await fetch("http://localhost:8080/listings/owner", {
  ...
  // ── fetch chats real ────────────────────────────────────────────────────────
  // service: getMyThreads → role OWNER → displayName = student.name
  useEffect(() => {
    if (!token) return;
    (async () => {
      try {
        ...
        setChats(raw.map((thread) => {
          ...
          return {
            id:          thread.id,
            // displayName dari service sudah = student.name untuk role OWNER
  ...
  const renderHome = () => (
    <div className="space-y-6">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              // nilai pesan baru dari data real
              { icon: MessageSquare, value: totalUnread,            label: "Pesan baru"     },
      ...
      {/* Messages preview — pakai data real, 3 teratas */}
      <div>
  ...
  // ── renderPesan — data real ──────────────────────────────────────────────────
  const renderPesan = () => (
    <div className="space-y-3 max-w-2xl">
      {/* Stat chips — dari data real */}
      ...
      {/* Chat list — klik navigate ke ChatDetailPage shared */}
  ...
  const renderAkun = () => (
    <div className="space-y-4 max-w-lg">
      ...
      {[
        ...
        { icon: Building2, label: "Properti Saya",     sub: `${listings.length} listing`, action: () => navigate("/owner/properti")     },
  ...
  return (
    <div className="flex min-h-screen bg-slate-50" style={{ fontFamily: "'Plus Jakarta Sans','Inter',sans-serif" }}>
      ...
      {/* pass unreadCount supaya badge di bottom nav akurat */}
