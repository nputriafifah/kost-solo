  ...
  return (
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'Plus Jakarta Sans','Inter',sans-serif" }}>
      ...
      <div className="relative rounded-b-3xl px-5 pt-12 pb-6 overflow-hidden"
        ...
        <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-blue-500/30" />
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-blue-200 text-xs">{schedules.length} jadwal terdaftar</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex bg-white rounded-2xl border border-slate-100 p-1 shadow-sm">
          {[{ id:"jadwal", label:"Daftar Jadwal" }, { id:"tambah", label:"+ Tambah Baru" }].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex-1 py-2.5 rounded-xl text-sm font-black transition-all ${tab === t.id ? "bg-blue-600 text-white shadow" : "text-slate-400"}`}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          schedules.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-10 text-center">
              ...
              <button onClick={() => setTab("tambah")}
                className="mt-3 bg-blue-600 text-white text-xs font-black px-4 py-2 rounded-xl">
                Tambah Jadwal
              </button>
            </div>
          ) : schedules.map(s => {
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-blue-500 font-bold">{s.kost}</p>
                    </div>
                  </div>
        ...
        {tab === "tambah" && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {TIME_SLOTS.map(t => (
                  <button key={t} onClick={() => setForm(p => ({...p, time: t}))}
                    className={`py-2.5 rounded-xl text-xs font-black border transition-all ${form.time === t ? "bg-blue-600 text-white border-blue-600" : "border-slate-200 text-slate-500 hover:border-blue-300"}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <button onClick={handleAdd}
              ...
              className="w-full bg-blue-600 disabled:bg-slate-200 disabled:text-slate-400 text-white font-black py-3.5 rounded-xl text-sm shadow-lg shadow-blue-100 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
