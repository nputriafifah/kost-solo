...
export default function CreateListingPage() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const resListing = await fetch("http://localhost:3000/listings/owner", {
        method: "POST",
        headers: {
      ...
      if (!resListing.ok) throw new Error("Gagal membuat listing");
      const listing = await resListing.json();
      ...
      if (!resRoom.ok) throw new Error("Gagal membuat tipe kamar");
      const roomRes = await resRoom.json();
      const roomId = roomRes.data.id;

      if (roomPhotos.length > 0) {
        const fd = new FormData();
        roomPhotos.forEach((f) => fd.append("photos", f));
        await fetch(`http://localhost:3000/room-types/${roomId}/photos`, {
          method: "POST",
  ...
  return (
    <>
      <style>{`
        ...
        .clp-btn-next:hover:not(:disabled) {
      ...
      >
        <div className="w-full max-w-2xl">
          ...
          {/* ── Progress steps ── */}
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm shadow-blue-900/5 p-4 mb-4">
          ...
          {/* ── Main card ── */}
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm shadow-blue-900/5 overflow-hidden">
            ...
            <div className="px-8 py-7 clp-step-fade" key={step}>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        ...
                        const active = form.genderType === val;
                        return (
                          <button
                            key={val}
                            type="button"
                            onClick={() => setForm({ ...form, genderType: val })}
                            className={active ? "clp-toggle-on" : "clp-toggle-off"}
                            style={{ padding: "10px 0", fontSize: 14, fontWeight: 600, textAlign: "center" }}
                          >
                            {label}
                          </button>
                        );
                      ...
                      {RULE_OPTIONS.map((rule) => {
                        ...
                        return (
                          <button
                            key={rule}
                            type="button"
                            onClick={() => setForm({ ...form, rules: toggleItem(form.rules, rule) })}
                            className={checked ? "clp-toggle-on" : "clp-toggle-off"}
                            style={{
                              padding: "10px 14px",
                              display: "flex",
                              alignItems: "center",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mb-3">
                      Klik pada peta untuk menentukan lokasi kost secara akurat.
                    </p>
                    ...
                    {latLng ? (
                      ...
                      >
                        ...
                        <span className="text-xs font-medium text-blue-700">
                          Lokasi dipilih: {Number(latLng.lat).toFixed(6)}, {Number(latLng.lng).toFixed(6)}
                        </span>
                      </div>
                    ) : (
              ...
              {step === 3 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <p className="text-sm text-slate-500">
                    Pilih semua fasilitas yang tersedia di dalam kamar.
                  </p>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <button
                        type="button"
                        onClick={() =>
                          setRoom({ ...room, availableCount: Math.max(0, Number(room.availableCount) - 1) })
                        }
                        style={{
                          width: 44,
                          height: 44,
                      ...
                      <input
                        ...
                        min="0"
                        className="clp-input"
                        style={{ textAlign: "center", fontSize: 28, fontWeight: 700, padding: "10px" }}
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setRoom({ ...room, availableCount: Number(room.availableCount) + 1 })
                        }
                        style={{
                          width: 44,
                          height: 44,
              ...
              {step === 6 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  <p className="text-sm text-slate-500">
                    Upload foto kamar yang menarik untuk meningkatkan minat penyewa.
                  </p>
                  ...
                  <label
                    style={{
                      ...
                      justifyContent: "center",
                      width: "100%",
                      padding: roomPhotos.length > 0 ? "24px 0" : "48px 0",
                      border: `2px dashed ${roomPhotos.length > 0 ? "#3b82f6" : "#cbd5e1"}`,
                      borderRadius: 14,
                      background: roomPhotos.length > 0 ? "#eff6ff" : "#f8faff",
                      cursor: "pointer",
                      transition: "all 0.2s",
                    }}
                  >
                    <span style={{ fontSize: 13, fontWeight: 600, color: roomPhotos.length > 0 ? "#1d4ed8" : "#475569" }}>
                      {roomPhotos.length > 0 ? `${roomPhotos.length} foto dipilih` : "Klik untuk upload foto"}
                    </span>
                    <span style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>
                      JPG, PNG — bisa pilih beberapa
                    </span>
                    <input
                      type="file"
                      multiple
            ...
            >
              {step > 1 ? (
                <button
                  ...
                  className="clp-btn-back flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm"
                >
                  <ChevronLeft size={16} />
                  Kembali
                </button>
