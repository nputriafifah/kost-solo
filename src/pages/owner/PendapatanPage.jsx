  ...
  return (
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'Plus Jakarta Sans','Inter',sans-serif" }}>
      ...
      <div className="relative rounded-b-3xl px-5 pt-12 pb-6 overflow-hidden"
        style={{ background: "linear-gradient(135deg,#064E3B 0%,#065F46 45%,#10B981 100%)" }}>
        <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-emerald-400/30" />
        <div className="relative z-10">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-emerald-200 text-sm font-semibold mb-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-emerald-200 text-xs">Saldo & penarikan kos kamu</p>
            </div>
          </div>
          {/* Balance card */}
          <div className="bg-white/15 backdrop-blur rounded-2xl p-4 border border-white/10">
            <p className="text-emerald-200 text-[11px] font-bold uppercase tracking-widest mb-1">Saldo Tersedia</p>
            <p className="text-white font-black text-3xl">Rp {fmt(balance)}</p>
            <div className="flex items-center justify-between mt-3">
              <div>
                <p className="text-emerald-200 text-[10px] font-semibold">Total pendapatan</p>
                <p className="text-white font-black text-sm">Rp {fmt(DUMMY_TOTAL)}</p>
              </div>
              <button onClick={() => setTab("tarik")}
                className="bg-white text-emerald-700 text-xs font-black px-4 py-2 rounded-xl flex items-center gap-1.5 active:scale-95 transition-transform shadow">
        ...
        <div className="flex bg-white rounded-2xl border border-slate-100 p-1 shadow-sm">
          {[
            ...
            <button key={t.id} onClick={() => { setTab(t.id); setStep("form"); }}
              className={`flex-1 py-2.5 rounded-xl text-xs font-black transition-all ${tab === t.id ? "bg-emerald-600 text-white shadow" : "text-slate-400"}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === "ringkasan" && (
          <>
            <div className="grid grid-cols-2 gap-3">
              {[
                { label:"Saldo tersedia",  value: `Rp ${fmt(balance)}`,      color:"text-emerald-600 bg-emerald-50" },
                { label:"Total pendapatan",value: `Rp ${fmt(DUMMY_TOTAL)}`,  color:"text-blue-600 bg-blue-50"       },
                { label:"Sudah ditarik",   value: `Rp ${fmt(DUMMY_TOTAL - balance)}`, color:"text-amber-600 bg-amber-50" },
                { label:"Pemasukan bulan ini", value: `Rp ${fmt(1_950_000)}`, color:"text-pink-600 bg-pink-50"      },
              ].map(s => (
              ...
              <div className="space-y-3">
                {DUMMY_INCOME.map(inc => (
                  <div key={inc.id} className="flex items-center gap-3">
                    ...
                    <p className="text-sm font-black text-emerald-600">+Rp {fmt(inc.amount)}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {step === "form" && (
              <div className="space-y-4">
                <div className="bg-emerald-50 rounded-xl px-4 py-3 flex items-center justify-between">
                  <span className="text-xs font-black text-emerald-600">Saldo tersedia</span>
                  <span className="text-base font-black text-emerald-700">Rp {fmt(balance)}</span>
                </div>
                ...
                <div>
                  ...
                  <select value={form.bank} onChange={e => setForm(p => ({...p, bank: e.target.value}))}
                    className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-50 bg-white transition-all">
                ...
                <div>
                  ...
                  <input value={form.account} onChange={e => setForm(p => ({...p, account: e.target.value}))}
                    ...
                    className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-50 transition-all" />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="0"
                      type="number"
                      className="w-full border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm font-semibold text-slate-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-50 transition-all" />
                  </div>
                  <div className="flex gap-2 mt-2">
                    {[100_000, 500_000, 1_000_000, 2_000_000].map(v => (
                      <button key={v} onClick={() => setForm(p => ({...p, amount: String(v)}))}
                        className="flex-1 py-1.5 rounded-lg border border-slate-200 text-[10px] font-black text-slate-500 hover:border-emerald-400 hover:text-emerald-600 transition-all">
                ...
                <button onClick={handleWithdraw}
                  className="w-full bg-emerald-600 text-white font-black py-3.5 rounded-xl text-sm shadow-lg shadow-emerald-100 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handleConfirm}
                    className="flex-1 py-3 rounded-xl bg-emerald-600 text-white text-sm font-black shadow-lg shadow-emerald-100 active:scale-[0.98]">
                    Ya, Tarik
                  </button>
                </div>
              </div>
            ...
            {step === "done" && (
              <div className="py-6 text-center space-y-3">
                ...
                <button onClick={resetForm}
                  className="mt-2 bg-emerald-600 text-white font-black px-6 py-2.5 rounded-xl text-sm">
                  Lihat Riwayat
                </button>
              </div>
            )}
            ...
            {DUMMY_HISTORY.map(h => (
              <div key={h.id} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">Berhasil</span>
                </div>
              </div>
            ))}
          </div>
