...
import { 
  ...
  Bell, Shield, HelpCircle, Edit3, X, Check, Camera 
} from "lucide-react";
import { useNavigate } from "react-router-dom";
import BottomNav from "../components/auth/BottomNav";

// Update menuItems dengan properti 'path' sesuai Route di App.js
const menuItems = [
  { 
    icon: Settings, 
...
export default function ProfilPage() {
  ...
  const userName = userData.fullname || "User Atap";
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-32 font-sans">
      ...
      <div className="mx-6 mb-8">
        <div className="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-slate-200/50 border border-white flex items-center gap-5 relative overflow-hidden">
          ...
          <div className="relative group">
            <div className="w-20 h-20 bg-indigo-600 rounded-[2rem] flex items-center justify-center text-white font-black text-2xl shadow-lg shadow-indigo-100 flex-shrink-0 transition-transform group-hover:scale-105">
              {initials}
            </div>
          ...
          <div className="flex-1 min-w-0 z-10">
            ...
            <button 
              ...
              className="mt-3 px-4 py-1.5 bg-slate-900 text-white text-[10px] font-black rounded-full uppercase tracking-widest hover:bg-indigo-600 transition-colors"
            >
              Ubah Profil
            </button>
          </div>
      ...
      <div className="mx-6 mb-8">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white overflow-hidden p-2">
          {menuItems.map(({ icon: Icon, label, color, bg, path }) => (
            <button 
              key={label} 
              onClick={() => navigate(path)} // Navigasi ke path masing-masing
              className="w-full flex items-center gap-4 px-4 py-4 hover:bg-slate-50 transition-all rounded-[1.8rem] group text-left"
            >
      ...
      <div className="mx-6">
        <button 
          ...
          className="w-full flex items-center justify-between px-8 py-5 bg-rose-50 rounded-[2rem] border border-rose-100 group active:scale-95 transition-all"
        >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-8 shadow-2xl relative overflow-hidden">
             ...
             <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setIsEditing(false)} className="p-2 bg-slate-100 rounded-full text-slate-400"><X size={20} /></button>
             </div>
             <form onSubmit={handleSaveProfile} className="space-y-5">
                <div>
                   ...
                   <div className="relative mt-1.5">
                      ...
                      <input 
                        type="text" 
                        className="w-full h-12 pl-12 pr-4 bg-slate-50 border-none rounded-2xl font-bold text-sm focus:ring-2 focus:ring-indigo-500 transition-all"
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 shadow-2xl">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 py-4 rounded-2xl border border-slate-100 text-slate-500 font-black text-xs uppercase tracking-widest transition-all">Batal</button>
              <button onClick={handleLogout} className="flex-1 py-4 rounded-2xl bg-slate-900 text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-slate-200 transition-all">Ya, Keluar</button>
            </div>
          </div>
        </div>
      )}
