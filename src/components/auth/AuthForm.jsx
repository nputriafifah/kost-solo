...
import { Search, Mail, Lock, Eye, EyeOff, Phone, User, CheckCircle2, ChevronLeft, ArrowRight, Shield } from "lucide-react";
...
console.log("RESULT:", result);
  ...
  return (
    <div className="fade-in">
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm font-semibold text-slate-400 hover:text-blue-600 mb-7 transition-colors">
      ...
      <form className="space-y-3" onSubmit={handleSubmit}>
        {!isLogin && (
          <Field label="Nama Lengkap" icon={<User size={15} className="text-slate-400" />}>
            <input name="name" type="text" placeholder="Budi Santoso" className={inputClass} value={form.name} onChange={handleChange} />
          </Field>
        )}
  ...
  <Field label="Nomor HP" icon={<Phone size={15} className="text-slate-400" />}>
    ...
    />
        ...
        >
          <input name="password" type={showPassword ? "text" : "password"} placeholder="Minimal 8 karakter" className={inputClass + " pr-10"} value={form.password} onChange={handleChange} />
        </Field>
        ...
        {!isLogin && (
          <Field
            ...
            icon={<Lock size={15} className="text-slate-400" />}
            rightEl={
              <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute transition-colors -translate-y-1/2 right-3 top-1/2 text-slate-400 hover:text-slate-600">
                {showConfirmPassword ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            }
          >
            <input name="confirmPassword" type={showConfirmPassword ? "text" : "password"} placeholder="Ketik ulang password" className={inputClass + " pr-10"} value={form.confirmPassword} onChange={handleChange} />
          </Field>
        )}

        {!isLogin && (
          <div className={`flex items-start gap-3 p-3.5 rounded-xl cursor-pointer border transition-all ${agreed ? "bg-blue-50 border-blue-200" : "bg-slate-50 border-slate-200"}`} onClick={() => setAgreed(!agreed)}>
            ...
            <p className="text-xs leading-relaxed text-slate-500">Saya menyetujui syarat dan ketentuan Kost Solo.</p>
          </div>
        )}

        {error && <div className="px-4 py-3 text-sm text-red-600 border border-red-200 rounded-xl bg-red-50">{error}</div>}

        {success && <div className="px-4 py-3 text-sm text-green-700 border border-green-200 rounded-xl bg-green-50">{success}</div>}

        <button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700 w-full py-3.5 rounded-xl font-bold text-white text-sm flex items-center justify-center gap-2 mt-1 disabled:opacity-70 transition-colors">
        ...
        {isLogin && (
          <p className="pt-1 text-xs text-center text-slate-400">
            <span onClick={() => navigate("/forgot-password")} className="font-medium text-blue-500 cursor-pointer hover:underline">
              Lupa password?
            </span>
          </p>
        )}
      ...
      <div className="flex items-center justify-center gap-2 pt-4 mt-6 border-t border-slate-100">
        ...
        <p className="text-[11px] text-slate-400 text-center">Data Anda aman & terenkripsi</p>
      </div>
    </div>
  );
}
