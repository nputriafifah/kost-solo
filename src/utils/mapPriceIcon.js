 ...
 * `pointer=false` → label harga TANPA ujung penunjuk & berlabuh di tengah,
 * dipakai di sisi user agar tidak terlihat menunjuk titik pasti (lokasi disamarkan).
 */
export function createPriceIcon(price, active = false, { pointer = true } = {}) {
  const label = `Rp ${formatMapPrice(price)}`;
  const width = Math.max(100, Math.min(168, label.length * 6.8 + 22));
  const height = 36;
  const anchorX = width / 2;
  const anchorY = pointer ? height + 4 : height / 2;
